...
float tim = 0.0; //stores the time in real life at the current frame
float lastT = 0.0; //stores the time in real life at the last frame
...
{
    if (tim != 0.0) // update the lastTime
        lastT = tim;
    tim = glutGet(GLUT_ELAPSED_TIME) / 1000.0;//gives you time in seconds
...
void timeIndexUpdate(int i) //update the new time index on the event log. i is the index
{
    if (i == 0)
    {
        updateEvents(">at " + to_string(tim) + " s: Time ratio now set to 1 s = 1 s");
    }
    else if (i == 1)
    {
        updateEvents(">at " + to_string(tim) + " s: Time ratio now set to 1 s = 1 min");
    }
    else if (i == 2)
    {
        updateEvents(">at " + to_string(tim) + " s: Time ratio now set to 1 s = 1 h");
    }
    else if (i == 3)
    {
        updateEvents(">at " + to_string(tim) + " s: Time ratio now set to 1 s = 1 day");
    }
    else if (i == 4)
    {
        updateEvents(">at " + to_string(tim) + " s: Time ratio now set to 1 s = 1 week");
    }
    else if (i == 5)
    {
        updateEvents(">at " + to_string(tim) + " s: Time ratio now set to 1 s = 1 month");
    }
    else if (i == 6)
    {
        updateEvents(">at " + to_string(tim) + " s: Time ratio now set to 1 s = 1 year");
    }
}
...
{
    if (i == 0)
        updateEvents(">at " + to_string(tim) + " s: Space ratio now set to 1 m = 1/10^9.0 m");
    else if (i == 1)
        updateEvents(">at " + to_string(tim) + " s: Space ratio now set to 1 m = 1/10^9.5 m");
    else if (i == 2)
        updateEvents(">at " + to_string(tim) + " s: Space ratio now set to 1 m = 1/10^9.7 m");
    else if (i == 3)
        updateEvents(">at " + to_string(tim) + " s: Space ratio now set to 1 m = 1/10^10.0 m");
    else if (i == 4)
        updateEvents(">at " + to_string(tim) + " s: Space ratio now set to 1 m = 1/10^10.25 m");
}

void sizeIndexUpdate(int i) //update the new size index on the event log. i is the index
{
    if (i == 0)
        updateEvents(">at " + to_string(tim) + " s: Planet size ratio now set to 1 m = 1/7500000 m");
    else if (i == 1)
        updateEvents(">at " + to_string(tim) + " s: Planet size ratio now set to 1 m = 1/10000000 m");
    else if (i == 2)
        updateEvents(">at " + to_string(tim) + " s: Planet size ratio now set to 1 m = 1/20000000 m");
    else if (i == 3)
        updateEvents(">at " + to_string(tim) + " s: Planet size ratio now set to 1 m = 1/40000000 m");
    else if (i == 4)
        updateEvents(">at " + to_string(tim) + " s: Planet size ratio now set to 1 m = 1/80000000 m");
}
...
void massControl(int i, float factor)//controls the mass of planets and the sun
{
    if ((i - 1) >= 0) //for planets, i-1 is to convert it into the index
    {
        if (planets[i - 1].mass < pow(10, 36) and factor > 1) {//if the mass is less than the maximum allowed
            ...
            updateEvents(">at " + to_string(tim) + ": " + planets[i - 1].name + "'s mass has increased to " + to_string(planets[i - 1].mass / pow(10, 24)) + " x 10^24 kg");
        }
        else if (factor > 1) {//if the mass is greater than the maximum allowed
            updateEvents(">at " + to_string(tim) + ": Maximum mass reached");
        }
        else if (planets[i - 1].mass > pow(10, 22) and factor < 1) {//if the mass is greater than the minimum allowed
            planets[i - 1].mass = planets[i - 1].mass * factor;//change the mass by the factor
            updateEvents(">at " + to_string(tim) + ": " + planets[i - 1].name + "'s mass has decreased to " + to_string(planets[i - 1].mass / pow(10, 24)) + " x 10^24 kg");
        }
        else { //if not
            updateEvents(">at " + to_string(tim) + ": Minimum mass reached");
        }
    ...
    if ((i - 1) == -1) //for sun
    {
        if (massSun < pow(10, 36) and factor > 1) {//if the mass is less than the maximum allowed
            ...
            updateEvents(">at " + to_string(tim) + ": Sun's mass has increased to " + to_string(massSun / pow(10, 24)) + " x 10^24 kg");
        }
            ...
            massSun = massSun * factor;//change the mass by the factor
            updateEvents(">at " + to_string(tim) + ": Sun's mass has decreased to " + to_string(massSun / pow(10, 24)) + " x 10^24 kg");
        }
        else {//if not
            updateEvents(">at " + to_string(tim) + ": Minimum mass reached");
        }
...
void updateRocket() //update the information of the rockets
{
    for (int i = 0; i < rockets.size(); i++)
    {
        ...
        float dtime = (tim - lastT) * TimeRatios[timeRatioIndex] / precisionFactor;//calculate the dt
        ...
        for (int it = 0; it < precisionFactor; it++)//for the number of dt
        {
            ...
            {
                updateEvents(">at " + to_string(tim) + " s: Rocket " + rockets[i].name + " has flown into the deep space and will never return.");//update event log
            ...
            {
                updateEvents(">at " + to_string(tim) + " s: Rocket " + rockets[i].name + " has crashed into the sun.");//update event log
                removee = true;//remove it
                goto label;//skip the rest
            }
            ...
            for (int i2 = 0; i2 < planets.size(); i2++)//for each planet
            {
                ...
                if (r < planets[i2].rr / 2)
                {
                    ...
                    updateEvents(">at " + to_string(tim) + " s: Neglecting air friction, Rocket " + rockets[i].name + " has landed (or crashed) on " + planets[i2].name + " with a speed of " + to_string(v) + " km/s relative to the planet");
...
void createRocket() //create a new rocket
{
    ...
    //update on event
    updateEvents(">at " + to_string(tim) + " s: Rocket " + r.name + " is created on " + planets[i].name);
...
void updateAcceleration(float& ax, float& ay, float& az, int i) //update the acceleration of a planet due to other planets
{
    for (int i2 = 0; i2 < planets.size(); i2++)//for each planet
    {
        if (i != i2)//for other planets
        {
            ...
            {
                updateEvents(">at " + to_string(tim) + " s: " + planets[i].name + " has crashed into " + planets[i2].name);//update the event
...
void updatePlanet()//update the information of the planets
{
    for (int i = 0; i < planets.size(); i++)
    {
        ...
        if (planets[i].exist)//as long as the planet exist
        {
            ...
            //change in time in second
            double dtime = (tim - lastT) * TimeRatios[timeRatioIndex] / precisionFactor;//calculate the dt
            for (int it = 0; it < precisionFactor; it++)//for the number of dt
            {
                ...
                {
                    updateEvents(">at " + to_string(tim) + " s: " + planets[i].name + " has flown into the deep space and will never return.");//update event
                ...
                if (r <= rSun * 1.5)
                {
                    updateEvents(">at " + to_string(tim) + " s: " + planets[i].name + " has crashed into the sun.");//update event
                    planets[i].exist = false;//planet no longer exists
                    break;
                }
...
void render() //this method draws everything
{
    ...
    universalTime();
    //display time
    realTime = realTime + (tim - lastT) * TimeRatios[timeRatioIndex];//real time in seconds
    if (nF3 > 0)
        displayTime();
...
void initialization()
{
    ...
    updateEvents(">at " + to_string(tim) + " s: Simulation Begins");
...
static void key(unsigned char key, int x, int y)//for normal keys
{
    ...
    if (!inputRocket)//while users are not inputing a rocket
    {
        ...
        case '0'://if pressed 0, select sun
            selectedPlanet = 0;
            updateEvents(">at " + to_string(tim) + " s: Selected Sun");//update the event log
            break;

        case '1'://if pressed 1, select mercury
            selectedPlanet = 1;
            updateEvents(">at " + to_string(tim) + " s: Selected Mercury");//update the event log
            break;

        case '2'://if pressed 2, select venus
            selectedPlanet = 2;
            updateEvents(">at " + to_string(tim) + " s: Selected Venus");//update the event log
            break;

        case '3'://if pressed 3, select earth
            selectedPlanet = 3;
            updateEvents(">at " + to_string(tim) + " s: Selected Earth");//update the event log
            break;

        case '4'://if pressed 4, select mars
            selectedPlanet = 4;
            updateEvents(">at " + to_string(tim) + " s: Selected Mars");//update the event log
            break;

        case '5'://if pressed 5, select Jupiter
            selectedPlanet = 5;
            updateEvents(">at " + to_string(tim) + " s: Selected Jupiter");//update the event log
            break;

        case '6'://if pressed 6, select Saturn
            selectedPlanet = 6;
            updateEvents(">at " + to_string(tim) + " s: Selected Saturn");//update the event log
            break;

        case '7'://if pressed 7, select Uranus
            selectedPlanet = 7;
            updateEvents(">at " + to_string(tim) + " s: Selected Uranus");//update the event log
            break;

        case '8'://if pressed 8, select Neptune
            selectedPlanet = 8;
            updateEvents(">at " + to_string(tim) + " s: Selected Neptune");//update the event log
            break;

        case '9'://if pressed 9, select Pluto
            selectedPlanet = 9;
            updateEvents(">at " + to_string(tim) + " s: Selected Pluto");//update the event log
            break;
        }
    }
...
void keySpecial(int key, int x, int y) //for special keys
{
    ...
    case GLUT_KEY_F5://if pressed f5, restart simulation
        updateEvents(">at " + to_string(tim) + " s: Simulation restarted");
        initialization();
        break;
    ...
    case GLUT_KEY_F10://if pressed f10, unselect the current selected planet
        if (selectedPlanet > 0)
            updateEvents(">at " + to_string(tim) + ": Unselected " + planets[selectedPlanet - 1].name);
        else if (selectedPlanet == 0)
            updateEvents(">at " + to_string(tim) + ": Unselected Sun");
        selectedPlanet = -1;
        break;
